...
import { buildFullName, extractLeadFormData } from "@/lib/helpers";
...
// Contact form submission (LeadForm)
export async function submitLeadForm(
  ...
  try {
    const { name, email, phone, countryCode, message } =
      validation.data;
...
// Contact modal submission (ContactModal)
export async function submitContactModal(
  prevState: FormState,
  formData: FormData
): Promise<FormState> {
  const rawData = extractLeadFormData(formData);
  const validation = contactModalSchema.safeParse(rawData);
  ...
  try {
    ...
    const payload: Partial<LeadModel> = {
      ...
      ...(countryCode && { Country_code: countryCode }),
      Message: message || "Contact request from modal",
    };
    ...
    console.error("Error submitting contact modal:", error);
    const errorMessage =
      error instanceof Error ? error.message : "Error submitting the form";
    return {
      success: false,
