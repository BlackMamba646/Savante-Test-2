...
export async function submitAboutPropertyInquiry(
  prevState: FormState,
  formData: FormData
): Promise<FormState> {
  const rawData = extractLeadFormData(formData);
  const validation = aboutPropertySchema.safeParse(rawData);
  ...
  try {
    ...
    const payload: Partial<LeadModel> = {
      ...
      Reason: `Property Information Request`,
      Message: message,
    };
    ...
    console.error("Error submitting about property inquiry:", error);
    const errorMessage =
