...
import { areaInquirySchema } from "@/lib/validations/lead.schema";
...
export async function submitAreaInquiry(
  prevState: FormState,
  formData: FormData
): Promise<FormState> {
  const rawData = extractLeadFormData(formData);
  const validation = areaInquirySchema.safeParse(rawData);
  ...
  try {
    const { name, email, phone, countryCode, area } = validation.data;
    ...
    const payload: Partial<LeadModel> = {
      ...
      ...(area && { Area: area }),
      Message: `Request for area information: ${area}`,
    };
    ...
    console.error("Error submitting area inquiry:", error);
    const errorMessage =
